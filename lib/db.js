...
// max: 10 — l'addon XXS Small Space autorise 45 connexions ; on en garde
// sous le pied pour le déploiement (l'ancienne instance tient les siennes
// jusqu'à la bascule) et les accès d'administration.
...
export async function migrate() {
  await pool.query(`
    ...
    -- Consentement explicite (accord API Strava) : date d'acceptation de la
    -- visibilité prénom/cases/dates par les autres joueurs. NULL = pas encore
    -- consenti : l'athlète est exclu des classements et de la carte partagée.
    ALTER TABLE athletes ADD COLUMN IF NOT EXISTS consent_at TIMESTAMPTZ;
    -- Langue de l'interface choisie par le joueur ('fr'|'en'|'it') : sert
    -- aussi côté serveur pour traduire les notifications push.
    ALTER TABLE athletes ADD COLUMN IF NOT EXISTS lang TEXT;
