        ...
        if (notify) {
          ...
          if (gained > 0) {
            sendToAthlete(athleteId, {
              title: "Territoires",
              body: `+${gained} case${gained > 1 ? "s" : ""} capturée${gained > 1 ? "s" : ""} 🏰`,
              tag: `capture-${a.id}`,
              url: "/",
...
export async function captureTerritory(athleteId, { notifySteals = true } = {}) {
  ...
  if (notifySteals) {
    ...
    if (stolen.size) {
      ...
      const attacker = at[0]?.firstname || "Un joueur";
      for (const [victim, n] of stolen) {
        sendToAthlete(victim, {
          title: "Ton territoire est attaqué !",
          body: `${attacker} t'a pris ${n} case${n > 1 ? "s" : ""} ⚔️`,
          tag: "raid",
          url: "/",
