...
import { geocodePendingTiles } from "./places.js";
import { sendToAthlete } from "./push.js";
...
// i18n des notifications push : la langue vient d'athletes.lang (choisie
// dans l'interface) ; défaut fr. Textes courts, {n}/{name} interpolés.
const NOTIF = {
  fr: {
    captured: (n) => `+${n} case${n > 1 ? "s" : ""} capturée${n > 1 ? "s" : ""} 🏰`,
    raidTitle: "Ton territoire est attaqué !",
    raid: (name, n) => `${name} t'a pris ${n} case${n > 1 ? "s" : ""} ⚔️`,
    someone: "Un joueur",
  },
  en: {
    captured: (n) => `+${n} tile${n > 1 ? "s" : ""} captured 🏰`,
    raidTitle: "Your territory is under attack!",
    raid: (name, n) => `${name} took ${n} tile${n > 1 ? "s" : ""} from you ⚔️`,
    someone: "A player",
  },
  it: {
    captured: (n) => `+${n} casell${n > 1 ? "e" : "a"} conquistat${n > 1 ? "e" : "a"} 🏰`,
    raidTitle: "Il tuo territorio è sotto attacco!",
    raid: (name, n) => `${name} ti ha rubato ${n} casell${n > 1 ? "e" : "a"} ⚔️`,
    someone: "Un giocatore",
  },
  es: {
    captured: (n) => `+${n} casilla${n > 1 ? "s" : ""} conquistada${n > 1 ? "s" : ""} 🏰`,
    raidTitle: "¡Tu territorio está bajo ataque!",
    raid: (name, n) => `${name} te ha robado ${n} casilla${n > 1 ? "s" : ""} ⚔️`,
    someone: "Un jugador",
  },
  de: {
    captured: (n) => `+${n} Kachel${n > 1 ? "n" : ""} erobert 🏰`,
    raidTitle: "Dein Revier wird angegriffen!",
    raid: (name, n) => `${name} hat dir ${n} Kachel${n > 1 ? "n" : ""} weggenommen ⚔️`,
    someone: "Ein Spieler",
  },
  pt: {
    captured: (n) => `+${n} quadrícula${n > 1 ? "s" : ""} conquistada${n > 1 ? "s" : ""} 🏰`,
    raidTitle: "O teu território está sob ataque!",
    raid: (name, n) => `${name} roubou-te ${n} quadrícula${n > 1 ? "s" : ""} ⚔️`,
    someone: "Um jogador",
  },
  nl: {
    captured: (n) => `+${n} tegel${n > 1 ? "s" : ""} veroverd 🏰`,
    raidTitle: "Je gebied wordt aangevallen!",
    raid: (name, n) => `${name} heeft ${n} tegel${n > 1 ? "s" : ""} van je afgepakt ⚔️`,
    someone: "Een speler",
  },
  pl: {
    // pluriel polonais : 1 pole, 2-4 pola, 5+ pól (règle 22/23/24 -> pola)
    captured: (n) => `+${n} ${plPole(n)} zdobyte 🏰`,
    raidTitle: "Twoje terytorium jest atakowane!",
    raid: (name, n) => `${name} zabrał ci ${n} ${plPole(n)} ⚔️`,
    someone: "Gracz",
  },
};
function plPole(n) {
  ...
  return d >= 2 && d <= 4 && (h < 12 || h > 14) ? "pola" : "pól";
}
async function athleteLang(athleteId) {
  const { rows } = await pool.query("SELECT lang FROM athletes WHERE id=$1", [athleteId]);
  return NOTIF[rows[0]?.lang] ? rows[0].lang : "fr";
}
...
async function syncAthlete(athleteId, full = true) {
  ...
  try {
    ...
    // notif « +N cases » seulement sur le chemin webhook (sortie fraîche) :
    // un backfill initial de 500 activités ne doit pas envoyer 500 notifs
    await processPendingActivities(athleteId, !full);
    ...
    geocodePendingTiles(); // enrichissement communes en arrière-plan
  } catch (e) {
...
async function processPendingActivities(athleteId, notify = false) {
  for (;;) {
    const { rows } = await pool.query(
    ...
    for (const a of rows) {
      ...
      if (latlngs?.length) {
        ...
        // RETURNING (xmax = 0) distingue les cases fraîchement conquises
        // des cases déjà possédées (upsert) : c'est le « +N » de la notif.
        ...
        if (notify) {
          const gained = up.reduce((t, r) => t + (r.fresh ? 1 : 0), 0);
          if (gained > 0) {
            athleteLang(athleteId)
              .then((l) =>
                sendToAthlete(athleteId, {
                  title: "Tilevore",
                  body: NOTIF[l].captured(gained),
                  tag: `capture-${a.id}`,
                  url: "/",
                })
              )
              .catch((e) => console.error("[push] capture:", e.message));
          }
        }
      }
...
export async function captureTerritory(athleteId, { notifySteals = true } = {}) {
  const { rows } = await pool.query(
  ...
  // RETURNING ne renvoie que les lignes réellement touchées (insert ou
  // vol effectif) : stolen_from y vaut l'ancien propriétaire — c'est la
  // liste exacte des victimes de cette passe.
    ...
    `INSERT INTO territory (z, x, y, owner_id, captured_at)
     ...
     ON CONFLICT (z, x, y) DO UPDATE
       SET owner_id = EXCLUDED.owner_id, captured_at = EXCLUDED.captured_at,
           stolen_from = territory.owner_id -- on retient à QUI la case est prise
       WHERE territory.owner_id <> EXCLUDED.owner_id
         AND territory.captured_at < EXCLUDED.captured_at
     RETURNING stolen_from, (xmax = 0) AS fresh`,
  ...
  if (notifySteals) {
    // une seule notif par victime, agrégée (« X t'a pris 12 cases »)
    const stolen = new Map(); // victimId -> nb de cases perdues
    for (const r of touched) {
      if (r.fresh || r.stolen_from == null) continue;
      const victim = Number(r.stolen_from);
      if (victim === athleteId) continue;
      stolen.set(victim, (stolen.get(victim) || 0) + 1);
    }
    if (stolen.size) {
      const { rows: at } = await pool.query(
        "SELECT firstname FROM athletes WHERE id=$1",
        [athleteId]
      );
      const attacker = at[0]?.firstname || null;
      for (const [victim, n] of stolen) {
        athleteLang(victim)
          .then((l) =>
            sendToAthlete(victim, {
              title: NOTIF[l].raidTitle,
              body: NOTIF[l].raid(attacker || NOTIF[l].someone, n),
              tag: "raid",
              url: "/",
            })
          )
          .catch((e) => console.error("[push] raid:", e.message));
      }
    }
  }
}
...
async function rebuildAthlete(athleteId) {
  ...
  await captureTerritory(athleteId, { notifySteals: false });
}
...
// Réparation ponctuelle (via once() de db.js) : maintenant que
// tilesForTrack/rasterizeTrace découpent les traces aux sauts GPS, on
// reconstruit cases, enclaves et territoire de chaque athlète depuis les
// polylines stockées — les fausses lignes (décrochages de montre déjà
// ingérés) disparaissent, et le territoire orphelin est élagué. Aucun
// appel Strava, aucune notif.
export async function repairGpsJumps() {
  const { rows } = await pool.query("SELECT id FROM athletes ORDER BY id");
  for (const r of rows) {
    const id = Number(r.id);
    try {
      await rebuildAthlete(id);
      console.log(`[repair-gps] athlète ${id} reconstruit`);
    } catch (e) {
      console.error(`[repair-gps] athlète ${id}:`, e.message);
    }
  }
}
