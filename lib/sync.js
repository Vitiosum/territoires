...
export async function captureTerritory(athleteId) {
  ...
  // L'encerclement ne compte QUE pour le mode territoire (choix produit) :
  // la carte perso (brouillard/couleurs) et ses stats ne montrent que ce qui
  // a été réellement parcouru, plus les petites enclaves de fillEnclaves.
  console.log(
...
export async function rebuildAthlete(athleteId) {
  const { rows } = await pool.query(
    `SELECT id, sport_type, polyline FROM activities
     WHERE athlete_id=$1 AND polyline IS NOT NULL`,
    [athleteId]
