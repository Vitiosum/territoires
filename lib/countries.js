...
function load() {
  ...
  const infoByName = new Map(
    wc.flatMap((c) => {
      // i18n : nom par langue de l'interface (fr par défaut, en, it)
      const info = {
        flag: c.flag,
        names: {
          ...
          nl: c.translations.nld?.common || c.name.common,
  ...
  countries = geo.features.map((f) => {
    ...
    return {
      // `name` reste le français (compat : clés de regroupement, appels existants)
      name: names.fr || f.properties.name,
      names: Object.fromEntries(
        ["fr", "en", "it", "es", "de", "pt", "nl", "pl"].map((l) => [l, names[l] || f.properties.name])
      ),
      flag: info?.flag || "",
      areaKm2: area(f) / 1e6,
